$(document).on('turbolinks:load', function () {
	if ($('canvas#sky').length) {
		...
		$('canvas#sky').on('mousemove', function (e) {
			...
			let pos = µ.getMousePos(c, e);

			let day = Math.floor((pos.x / 20 % 14) + 1) + Math.floor((pos.y / 20)) * 14;
...
µ.drawDays = function (response) {
	let days = new Set(response.days);
...
µ.getDays = function (event) {
	...
	let tags = $('#tag-selector').val() || [];
	let media_works = $('#media-work-selector').val() || [];
	$.ajax({
		type: 'GET',

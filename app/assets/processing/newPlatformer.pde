...
void levelSetup(char[][] levelLayout){
  boolean[][] checklist = new boolean[levelLayout.length][levelLayout[0].length];
    ...
    for(int j = 0; j < levelLayout[i].length; j++){
      if (checklist[i][j] == false){
        ...
        if (levelLayout[i][j] == 'P'){
           ...
           xSize += gridSize;
           ySize += gridSize;
           platformCheckY(j, i, levelLayout[i][j], checklist);
           platformCheckX(j, i, levelLayout[i][j], checklist);
...
void platformCheckY(int j, int i, char test, boolean[][] check){ // "test" allows the function to check for adjacent platforms of the same type
  
  if(i + 1 < levelLayout.length){
    if(levelLayout[i + 1][j] == test){
      ySize += gridSize;
      platformCheckY(j, i+1, test, check);
      check[i + 1][j] = true;
      //levelLayout[i + 1][j] = 'r'; //Overwriting the character keeps the slot from being read again and creating another platform.
    }                              //This is a lazy workaround, though, and must be changed in order to produce multiple levels.
  }
...
void platformCheckX(int j, int i, char test, boolean[][] check){
  if (j + 1 < levelLayout[i].length){
    if (levelLayout[i][j + 1] == test){
      xSize += gridSize;
        platformCheckX(j + 1, i, test, check);
        check[i][j + 1] = true;
        levelLayout[i][j + 1] = 'r';
